...
const [open,setOpen] = useState(false);
  ...
  return (
    <div>
  {/* <div className="bg-slate-900 fixed top-0 left-0 right-0 shadow-lg">
    <div className="md:flex md:justify-around container mx-auto lg:justify-between md:items-center md:p-2 p-0 rounded-lg">
        ...
        <ul className={`flex flex-col justify-center md:flex-row md:z-auto z-20 absolute md:static w-full md:w-auto items-center md:space-x-3 shadow-2xl md:shadow-none bg-green-500 md:bg-transparent text-white md:text-black h-screen md:h-auto transition-all duration-500 ease-in md:py-0 space-y-10 md:space-y-0 text-center md:text-start ${open ? "left-0": "left-[-1024px]"}`}>
        {
            menuLinks.map(menu => {
                return (
                <li className='md:text-slate-200' key={menu.name}>{menu.name}</li>
                )
            })
        }
        </ul>
          <div className='absolute top-[20px] right-[60px] md:static'>
          {/* <label htmlFor="toggleB" className="flex items-center cursor-pointer">
          <div className="relative">
            <input onClick={()=>console.log("Hi")} type="checkbox" id="toggleB" className="sr-only"/>
